    <div class="question_sort">
    <h4><a name="q1">1. 请您写下您对A、B、C、D四个单位受损设备的抢修顺序安排。</a></h4> 
    <div class="waiting_item"><strong>选项:</strong><div value="A" class="item">A</div> 
    <div value="B" class="item">B</div> 
    <div value="C" class="item">C</div> 
    <div value="D" class="item">D</div></div>
    <div class="choosed_item"><strong>答案:</strong></div> </div>

    <div class="question_sort">
    <h4><a name="q2">2. 哪一个因素您在做抢修决策时考虑的最多</a></h4> 
    <div class="waiting_item"><strong>选项:</strong><div value="A" class="item">A. 位置</div> 
    <div value="B" class="item">B.设备受损单位</div> 
    <div value="C" class="item">C.备用电源情况</div> 
    <div value="D" class="item">D. 抢修时间</div></div>
    <div class="choosed_item"><strong>答案:</strong></div> </div>

